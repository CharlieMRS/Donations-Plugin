...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || array_key_exists( 'woocommerce/woocommerce.php', get_site_option('active_sitewide_plugins') ) ):
    ...
    function tallgrass_woo_enqueue_scripts() {
        ...
        wp_enqueue_script('select2OptionPicker', plugins_url( '/js/select-to-option-picker.js', __FILE__ ), array('jquery'), '1.2.2', true);

        //wp_enqueue_script('thisser-scripts', plugins_url( '/js/update-variation-on-checkout.js', __FILE__ ), array('add-to-cart-variation_ajax'), false, true);
    ...
    //add_action( 'woocommerce_before_checkout_form', 'tg_add_product_to_checkout' );
    ...
    function product_on_checkout_page_func( ) {
        ...
        <div id = "subscribeDonationWrapper">
            <input type="checkbox" id="subscribeDonation" name="subscribeDonation" value="subscribeDonation" <?php echo (get_class(array_values(WC()->cart->cart_contents)[0]['data']) != "WC_Product_Variation") ? 'checked' : '';?> >
        ...
        //$product = new WC_Product_Variation(79);

        //$GLOBALS['product'] = new WC_Product_Variation(79);
        //$chunkOfDataHere = new WC_Structured_Data;
        //$chunkOfDataHere->generate_product_data();
        //print_r($chunkOfDataHere->get_data());
        ...
        if ($wp_query->have_posts()) :
            while ($wp_query->have_posts()) :
                ...
                global $woocommerce;
                //print_r($woocommerce->structured_data);

                //calling the generate_product_data hook makes page hang
    ...
    function woo_custom_add_to_cart( $cart_item_data ) {
        ...
        //do_action( 'woocommerce_checkout_update_order_review', $array, $int );
    ...
    function tg_woo_debug_func( ) {
        ...
        echo '<pre>'; print_r(get_class(array_values(WC()->cart->cart_contents)[0]['data'])); echo '</pre>';
